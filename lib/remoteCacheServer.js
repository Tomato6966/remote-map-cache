...
class RemoteCacheServer extends EventEmitter {
            ...
            .on("request", async (request, response, client) => {
                ...
                */
                ...
                // write and RECEIVE DAta
                    ...
                    if(request.data && Array.isArray(request.data) && request.key.includes("ALLDATABASE_") && request.key.includes("_ALLDATABASE")) {
                        // Set cache of all subvalues
                        for(const d of request.data) {
                            this.cache.set(request.key.split("ALLDATABASE_")[0], d.data);
                        }
                    }
                    if(d instanceof Map) {
                        await this.sendCacheUpdate();
    ...
    async sendCacheUpdate() {
        return await this.server.survey({ requestAChange: true, wholeCache: [...this.cache.entries()] }).then(results => {
            console.log("DatabaseCacheServer responses of the Cache Update: ", results.map(r => r.status))
            return true;
        }).catch(console.error);
    }
} 
